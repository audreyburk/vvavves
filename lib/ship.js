...
Ship.prototype.move = function(){
  for(let i = 0; i < this.wave.points.length; i++){
    ...
    if(point.x > this.x){
      ...
      const leftWeight = left / total;
      const rightWeight = right / total;

      console.log(leftWeight);
      console.log(rightWeight);
      console.log(leftWeight + rightWeight); // adds to one appropriately
      this.y = (point.y * leftWeight + prevPoint.y * rightWeight);
